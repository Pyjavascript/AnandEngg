...
exports.createSubmission = async (data) => {
  const [result] = await db.query(
    `INSERT INTO report_submissions (template_id, employee_id, inspection_date, shift, status, created_at)
     VALUES (?, ?, ?, ?, ?, NOW())`,
    [data.template_id, data.employee_id, data.inspection_date || null, data.shift || null, data.status || 'draft']
  );
  return result;
};
...
exports.getById = async (id) => {
  const [rows] = await db.query(`SELECT * FROM report_submissions WHERE id = ?`, [id]);
  return rows[0];
};
...
exports.updateInspectorReview = async (submissionId, inspectorId, observation, remarks) => {
  const [result] = await db.query(
...
exports.updateManagerReview = async (submissionId, managerId, remarks, approved) => {
